...
#%% 1. loda train data from file
    ## 1.1 读取文件
    ### 读取嵌入矩阵---------------------------------------
    with open('word2idx_embedMatrix_glove_word2vec.pkl', 'rb') as f:
    ...
    ### 读取训练集和测试集---------------------------------
    with open('train_data_model_glove_word2vec.pkl', 'rb') as f:
        train_data_notitle = pickle.load(f)
    with open('train_data_model_hastitle_glove_word2vec.pkl', 'rb') as f:
        train_data_hastitle = pickle.load(f)

#%% 2.load test data, 载入测试集数据，用于预测
    with open('test_data_model_glove_word2vec.pkl', 'rb') as f:
        test_data_notitle = pickle.load(f)
    with open('test_data_model_hastitle_glove_word2vec.pkl', 'rb') as f:
...
#    txt_predict_hastitle_test = model_txt_notitle.predict(test_data_hastitle['txt'])
    ...
    ## 3.3 保存预测结果
    with open(r'./predict_hastitle_glove_word2vec.pkl', 'wb') as f:
        pickle.dump(title_predict_hastitle_test,f)
    with open(r'./predict_ignore_title_glove_word2vec.pkl', 'wb') as f:
        pickle.dump(txt_predict_notitle_test,f)
        

    with open(r'./train_predict_hastitle_glove_word2vec.pkl', 'wb') as f:
        pickle.dump(title_predict_hastitle,f)
    with open(r'./train_predict_ignore_title_glove_word2vec.pkl', 'wb') as f:
